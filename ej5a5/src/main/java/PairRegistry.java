 ...
 * --------------------------------------------
 * Se desea implementar un sistema que permita asociar un nombre (por ejemplo,
 * de una persona o producto)con una cifra numérica (por ejemplo, unidades
 * vendidas, puntuación, ingresos...).
 *
 * Para ello se debe:
 * 1. Crear una clase Pair<T, U> que almacene dos valores relacionados: uno de
 * tipo T (nombre), otro de tipo U (cifra).
 * 2. Crear una clase PairRegistry<T, U> que contenga una lista de objetos
 * Pair<T, U> y permita las siguientes operaciones:
 *
 ...
 * ---------------------------------------
 * Es vol implementar un sistema que permeti associar un nom (per exemple, d’una
 * persona o producte) amb una xifra numèrica (per exemple, unitats venudes,
 * puntuació, ingressos...).
 *
 * Per això cal:
 * 1. Crear una classe Pair<T, U> que emmagatzemi dos valors relacionats: un de
 * tipus T (nom), i un altre de tipus U (xifra).
 * 2. Crear una classe PairRegistry<T, U> que contingui una llista d’objectes
 * Pair<T, U> i permeti les operacions següents:
 *
 * - void addPair(T first, U second)
...
public class PairRegistry<T, U> {
    ...
    private List<Pair<T, U>> pairsList;
    ...
    public U getValueByKey(T key) {
        for (Pair<T, U> pair : pairsList) {
            if (Objects.equals(pair.first, key)) {
                return pair.second;
            }
        }
        return null;
    ...
    public U sumValues() {
        ...
        for (Pair<T, U> pair : pairsList) {
            if (pair.getSecond() instanceof Integer) {
                sum += (Integer) pair.getSecond();
            } else {
                return null;
            }
        }

        return (U) Integer.valueOf(sum);
    }
